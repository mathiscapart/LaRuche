"""Construction et écriture des événements honeypot (Epic 2).

Chaque événement est conforme à docs/event.schema.json (HoneypotEvent v1).
Sortie : une ligne JSON par événement (JSON Lines) sur stdout (logs Docker) et,
optionnellement, dans un fichier tail par Filebeat/Fluent Bit puis expédié au
SIEM. Conventions alignées sur le honeypot SSH (honeypots/ssh/events.py).
"""
...
from app.config import HONEYPOT_HOST, LOG_FILE, SCHEMA_VERSION
...
def normalize_ipv4(addr: str) -> str:
    """Ramène une adresse à la forme IPv4 attendue par le schéma (cf. SSH).

    Déplie les IPv4 mappées IPv6 (``::ffff:127.0.0.1``) et rabat le loopback
    IPv6 sur ``127.0.0.1`` pour respecter ``format: ipv4``.
    """
...
def build_event(
    *,
    ...
    honeypot_host: str = HONEYPOT_HOST,
) -> dict[str, Any]:
    ...
    event: dict[str, Any] = {
        ...
        "payload": payload,
        "meta": {"honeypot_host": honeypot_host, "schema_version": SCHEMA_VERSION},
    }
...
# Sink partagé par les middlewares et les routes (le fichier reste ouvert).
sink = EventSink(LOG_FILE)


def emit(event: dict[str, Any]) -> None:
    """Émet via le sink partagé (API conservée pour les routes/middlewares)."""
    sink.emit(event)
