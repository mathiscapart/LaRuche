...
- ``SSH_BIND_PORT``            port d'écoute "standard" (défaut ``2222``)
- ``SSH_ALT_BIND_PORT``        port d'écoute "alternatif" (défaut ``2223``)
- ``SSH_PUBLIC_PORT``          port public derrière SSH_BIND_PORT (défaut ``22``)
- ``SSH_ALT_PUBLIC_PORT``      port public derrière SSH_ALT_BIND_PORT (défaut ``2222``)
...
class Config:
    ...
    # Port interne d'écoute -> port public visé par l'attaquant. Deux listeners
    # distincts permettent de différencier le 22 "standard" du 2222 "alternatif"
    # dans les events (champ dst_port), info sinon perdue par le NAT Docker.
    port_map: dict[int, int] = field(default_factory=lambda: {2222: 22, 2223: 2222})
...
def load_config() -> Config:
    ...
    alt_bind_port = int(os.getenv("SSH_ALT_BIND_PORT", "2223"))
    public_port = int(os.getenv("SSH_PUBLIC_PORT", "22"))
    alt_public_port = int(os.getenv("SSH_ALT_PUBLIC_PORT", "2222"))
    return Config(
        bind_host=os.getenv("SSH_BIND_HOST", "0.0.0.0"),  # noqa: S104  # nosec B104
        bind_port=bind_port,
        port_map={bind_port: public_port, alt_bind_port: alt_public_port},
